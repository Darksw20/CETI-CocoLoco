...
$base=mysqli_query($con,"SELECT DISTINCT SubClass AS Busqueda,(SELECT IFNULL(SUM(B.Amount),'1') FROM Stocktaking AS A INNER JOIN Transaction AS B ON A.ID=B.Stocktaking_ID WHERE A.Class=Busqueda AND A.User_User_Name='$Usuario' AND B.DATE BETWEEN '$fecha1semana' AND '$fechahoys' ) AS Cantidad_De_Ventas FROM Stocktaking ORDER BY Cantidad_de_ventas DESC LIMIT 3");
$numeral1="";
